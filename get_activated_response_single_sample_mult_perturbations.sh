...
# Map the microarray data onto the unweighted network to get an activated response network.
...
python activated_response_single_sample_mult_perturbations.py ${data_fname} ${perturbation_sample} ${control_sample} ${unweighted_nw_fname} ${percentile} ${path_length_thresh} ${num_trials} ${out_dir}/${response_nw_fname} ${out_dir}/temp/Pij
...
python extract_zscore_network.py ${out_dir}/${response_nw_fname} ${out_dir}/Pij_zscores_fdr.txt ${qscore_thresh} ${out_dir}/${topnet_fname}
