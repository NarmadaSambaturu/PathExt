# Code to create a repressed response network given inputs PPI and gene expression data,
...
# NOTE: This code assumes the microarray data has only 1 sample per condition, but there are
# 	multiple perturbed conditions available. 
#	Column 0 -> Gene symbols, 
# 	Columns 1 through m -> normalised SI in the conditions under study. One of them is the control.
#	Header for the control and the perturbation of interest are given as user inputs.
...
def combine_data_get_sp_paths_costs_repressed(G_unweighted, SI, response_nw_fname, paths):
...
Pij, SI = combine_data_get_sp_paths_costs_repressed(G_unweighted, SI, response_nw_fname, []) # Give an empty list as the paths argument
...
for i in range(num_trials):
	...
	Pij, randomized_SI = combine_data_get_sp_paths_costs_repressed(G_unweighted, randomized_SI, response_nw_fname, list(Pij.index))
